...
#define FORij(i, j, s, t, ...) \
  for (int i = s; i < A r; i++) __VA_ARGS__ for (int j = t; j < A c; j++)
#define FOR(...) FORij(i, j, 0, 0, __VA_ARGS__)
#define SIZE(N, M) (N) * sizeof(double *) + (N) * (M) * sizeof(double)
...
int s21_create_matrix(int rows, int columns, matrix_t *A) {
  ...
  if (!(Am = (double **)malloc(SIZE(rows, columns)))) return 2;
    FOR({
    Am[i] = (double *)(Am + rows) + i * columns;) Am[i][j] = 0;}
    return 0;
}
...
#define CHECK(er1, er2, ...)                                           \
  if (bred(A) || bred(B)) return er1;                                  \
  if (wsecond(FIND __VA_ARGS__(A r != B r || A c != B c), A c != B r)) \
    return er2;
...
EVOLVE(eq_matrix, CHECK(FAILURE, FAILURE),
       if (fabs(Am[i][j] - Bm[i][j]) > EPS) return FAILURE, SUCCESS,
       matrix_t *B);

#define MATRIX_SLACK(name, extra, op, ind1, ind2)                          \
  EVOLVE(name##_matrix, CHECK(1, 2, extra);                                \
         s21_create_matrix(A r, B c, result),                              \
         extra rm[i][ind2] += Am[i][j] op Bm[ind1][ind2], 0, matrix_t * B, \
         matrix_t * result)
...
#define shortcut(n, m)   \
  if (bred(A)) return 1; \
  s21_create_matrix(A n, A m, result)
EVOLVE(mult_number, shortcut(r, c), rm[i][j] = Am[i][j] * number, 0,
       double number, matrix_t *result);
EVOLVE(transpose, shortcut(c, r), rm[j][i] = Am[i][j], 0, matrix_t *result);

#define maxN 64
#define principi_programirovania \
  matrix_t *B = A;               \
  CHECK(1, 2, xxx);
...
#define no_goto(...)                  \
  {                                   \
    memcpy(Am, temp, SIZE(A c, A c)); \
    __VA_ARGS__;                      \
  }
int s21_determinant(matrix_t *A, double *result) {
  principi_programirovania;
  char temp[SIZE(maxN, maxN)];
  memcpy(temp, Am, SIZE(A c, A c));
  *result = 1;

  FORij(k, l, 0, k) if (fabs(Am[l][k]) > EPS) {
    *result *= Am[l][k] * (l != k ? -1 : 1);
    FORij(i, j, k, k + 1, if (i != l)) Am[i][j] -=
        Am[l][j] * Am[i][k] / Am[l][k];
