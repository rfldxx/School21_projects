...
bool verdict(char *line, int *widespread, int pattern_count,
             regex_t *eryndushka, regmatch_t *bounds, int *absolute_pos,
             bool is_first) {
  ...
  for (int i = 0; i < pattern_count; i++) {
    if (is_first || absolute_pos[i] == *widespread) {
      if (is_first)
        ...
        absolute_pos[i] += (bounds[i].rm_eo - bounds[i].rm_so);
    ...
    if (next_widespread > absolute_pos[i]) {
      next_widespread = absolute_pos[i];
    }
  }
...
void clever_print(FILE *file, char *filename, bool *commands, int longest_name,
                  int digits_lines, int pattern_count, regex_t *eryndushka,
                  bool ultra) {
  char *line = NULL;
  size_t capacity = 0;

  // cute_offset используется только с CUTEE
  int cute_offset = (digits_lines + (digits_lines > 0)) +
                    (!commands[6] && longest_name ? abs(longest_name) + 1 : 0);
  ...
  while ((int)(l = getline(&line, &capacity, file)) != -1) {
    ...
    int branch =
        ultra
            ? 2 * !commands[2]
            : (pattern_count || CUTEE) &&
                  (commands[2] ^ verdict(line, &widespread, pattern_count,
                                         eryndushka, bounds, absolute_pos, 1));
    //такой кринж, потому что если передано 0 шаблонов то grep во всех случаях
    //ничго не выводит
    ...
    if (branch) {
      ...
      if (ultra || !(commands[9] && !commands[2]))
        printf("%s%s", !ultra || !commands[2] ? line : "",
               line[l - 1] != '\n' ? "\n" : "");
    }
      ...
      do {
        for (int i = 0; i < pattern_count; i++) {
          if (bounds[i].rm_so != -1 && absolute_pos[i] == widespread) {
            ...
            printf("%*.*s\n", (need_ofset++ && CUTEE) * (cute_offset + volume),
                   volume, line + absolute_pos[i]);
          }
        }
      } while (verdict(line, &widespread, pattern_count, eryndushka, bounds,
                       absolute_pos, 0));
    }
  }
...
int main(int argc, char *argv[]) {
    ...
    if (files_type[i] == 3) {
      ...
      while ((int)(l = getline(&pattern_line, &pattern_capacity, file)) != -1) {
        if (l && pattern_line[0] != '\n') {
          ...
          if (regcomp(eryndushka + (reg_pos++), pattern_line,
                      commands[1] * REG_ICASE)) {
            reg_pos--;
            bad_pattern = 1;
          }
        } else {
          ultra = 1;  //если ultra стала равна 1, то pattern_count дальше не
  ...
  longest_name = (CUTEE ? longest_name : 1) *
                 (2 * (!commands[6] && re_gyve(files_count) > 1) - 1);
