...
Понятном что в худшем случае эта операция может занять $\ln n$ итераций.

Оценем сколько требуется, чтобы проитерироваться плюс-плюсом от $\text{b} \text{egin}()$ до $\text{end}()$. \
Будем рассматривать идеальное дерево с $n = 2^h$ вершинами:
